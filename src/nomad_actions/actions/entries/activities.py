...
@activity.defn
async def save_dataset(data: SaveDatasetInput) -> None:
    """
    ...
    # match with regex if filename is already present in the upload_files directory
    # if true, find an available filename by appending an incrementing part number
    part_number = 1
    if upload_files.raw_path_exists(filename):
        while True:
            name, ext = os.path.splitext(filename)
            new_filepath = f'{name}({part_number}){ext}'
            if not upload_files.raw_path_exists(new_filepath):
