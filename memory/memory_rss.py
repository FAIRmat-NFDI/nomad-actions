...
def save_rss_results(
    ...
    result = {
        ...
        'num_entries': [interval['num_entries'] for interval in workflow_intervals],
...
def execute_workflow(data: ExportEntriesUserInput) -> tuple[float, str, str]:
    ...
    print(f'Started workflow for page size: {data.search_settings.max_entries}')

    action_result = wait_and_get_result(
        action_instance_id, data.user_id, wait=WORKFLOW_COMPLETION_WAIT_SECONDS
    )
    print(
        f'Page size: {data.search_settings.max_entries}, Time taken: {action_result["workflow_duration"]}'
    )
...
def execute_workflows_serial(data: list[ExportEntriesUserInput]):
    ...
    for index, d in enumerate(data, start=1):
        ...
        workflow_intervals.append(
            {
                'workflow': index,
                'num_entries': d.search_settings.max_entries,
...
if __name__ == '__main__':
    data_file = Path(__file__).parent / 'export_entries_user_input.json'
    data: list[ExportEntriesUserInput] = []
    with open(data_file) as f:
        export_entries_user_input = ExportEntriesUserInput.model_validate(json.load(f))
    ...
    for file_format in ['parquet']:
        for max_entries in [2000] * 2:
            ...
            model.export_settings.file_format = file_format
            data.append(model)
    execute_workflows_serial(data)
